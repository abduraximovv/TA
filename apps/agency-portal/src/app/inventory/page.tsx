...
export default function InventoryPage() {
  ...
  const handleSubmit = async (values: ServiceFormValues) => {
    ...
    if (editingService) {
      ...
      if (error) throw new Error(error.message);
      setToastMessage("Listing created.");
    }
    await fetchServices();
  };
    ...
    <div className="p-8">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage the packages and experiences your agency offers.</p>
        </div>
      ...
      {error ? (
        <div className="bg-red-50 text-red-600 p-4 rounded-lg">Failed to load inventory: {error}</div>
      ) : isLoading ? (
        ...
        <Card className="p-12 text-center rounded-xl border-dashed border-2 border-gray-200 bg-gray-50/50 shadow-none">
          <p className="text-gray-500 font-medium">You haven't added any listings yet.</p>
      ...
      {session?.user?.id && (
        <ServiceFormModal
          ...
          title={editingService ? "Edit Listing" : "Add Listing"}
