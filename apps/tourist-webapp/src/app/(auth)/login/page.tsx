...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Let standard Supabase session handler redirect or trigger state change
    } catch (err: unknown) {
      // Safe user-friendly error checking to avoid leaking database internals
      const message = err instanceof Error ? err.message : String(err);
      if (message.includes("Invalid login credentials") || message.includes("invalid_credentials")) {
        setError("Invalid email or password. Please try again.");
      } else if (message.includes("NetworkError") || message.includes("fetch")) {
        setError("Network connection timed out. Please check your internet connection.");
      } else {
